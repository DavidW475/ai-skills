...
Examples:
  ...
  ai-skills available`,
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			return runSearch(cmd, plainHTTP)
		},
	}
}

func runSearch(cmd *cobra.Command, plainHTTP bool) error {
	ctx := cmd.Context()
	...
	for _, src := range sf.Sources {
		if printSearchSource(cmd, ctx, w, src, lf, plainHTTP) {
			found = true
		}
	}
	w.Flush()
...
func printSearchSource(cmd *cobra.Command, ctx context.Context, w io.Writer, src string, lf *lockfile.LockFile, plainHTTP bool) bool {
